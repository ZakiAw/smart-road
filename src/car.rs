...
impl<'a> Car<'a> {
    ...
    pub fn update_position(&mut self, others: &[Car]) {
        ...
        let near = bounds.0.contains(&self.position.0) && bounds.1.contains(&self.position.1);
        ...
        if near && self.lane != Lane::Right && self.lane != Lane::Air {
            for c in others {
                if c.id != self.id
                    && c.in_bounds(&bounds)
                    && c.id < self.id
